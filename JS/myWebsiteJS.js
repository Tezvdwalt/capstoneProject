$(document).ready(function () {
  ...
  $(document).on("click", ".save_btn", function () {
    ...
    let sectionLink =
      window.location.href.split("#")[0] + "#section-" + sectionId;
    ...
    if (!savedItems.some((item) => item.id === sectionId)) {
      ...
      alert(
        `"${sectionTitle}" saved for later! You now have ${savedItems.length} item(s) saved.`
      );
    } else {
  ...
  $(document).on("click", ".delete-comment", function () {
    ...
    if (comments[childName]) {
      comments[childName].splice(index, 1); // Remove comment at that index

      if (comments[childName].length === 0) {
        delete comments[childName]; // Remove empty comment arrays from storage
      }
